...
//put the results error messages in a vector
void Display::push_res_errors(const std::string str)
{
	Res_Errors.push_back(str);
}
...
void Display::print_file_missing(File_Type type, Game_Mode mood)
{
    Display& display = Display::get_instance();
    if (mood == Game_Mode::SAVE) {
        switch (type) {
        case File_Type::STEPS:
            display.print_layout(step_file_missing_save);
            break;
        case File_Type::RES:
            display.print_layout(result_file_missing_save);
            break;
        default:
            break;
        }
    }
    else {//mood == Game_Mode::LOAD
        switch (type) {
        case File_Type::STEPS:
            display.print_layout(step_file_missing_load);
            break;
        case File_Type::RES:
            display.print_layout(result_file_missing_load);
            break;
        default:
            break;
        }
...
* @param positions The positions to print the messages.
*/
...
bool Display::error_message(const std::vector<Board::Err_Code>& errors) const {
    ...
    // Mechanism to print the error messages
    for (auto err : errors) {
        gotoxy(3, row);
        switch (err) {
...
char Display::result_layout[SCREEN_HEIGHT][SCREEN_WIDTH + 1]
{
    //01234567890123456789012345678901234567890123456789012345678901234567890123456789
  R"!(  ..............................................................................)!", // 0
  R"!(  :8888888b.  8888888888 .d8888b.  888     888 888    88888888888 .d8888b.     :)!", // 1
  R"!(  :888   Y88b 888       d88P  Y88b 888     888 888        888    d88P  Y88b    :)!", // 2
  R"!(  :888    888 888       Y88b.      888     888 888        888    Y88b.         :)!", // 3
  R"!(  :888   d88P 8888888    "Y888b.   888     888 888        888     "Y888b.      :)!", // 4
  R"!(  :8888888P"  888           "Y88b. 888     888 888        888        "Y88b.    :)!", // 5
  R"!(  :888 T88b   888             "888 888     888 888        888          "888    :)!", // 6
  R"!(  :888  T88b  888       Y88b  d88P Y88b. .d88P 888        888    Y88b  d88P    :)!", // 7
  R"!(  :888   T88b 8888888888 "Y8888P"   "Y88888P"  88888888   888     "Y8888P"     :)!", // 8
  R"!(   .............................................................................)!", // 9
  R"!(                                                                                )!", // 10
  R"!(                                                                                )!", // 11
  R"!(                                                                                )!", // 12
...
char Display::step_file_missing_load[SCREEN_HEIGHT][SCREEN_WIDTH + 1]
{
    //01234567890123456789012345678901234567890123456789012345678901234567890123456789
  R"!(                                                                                )!", // 0
  R"!(                                                                                )!", // 1
  R"!(      **************************************************************            )!", // 2
  R"!(      *____ ___ ____ ___  ____    ____ _ _    ____    _ ____       *            )!", // 3
  R"!(      *[__   |  |___ |__] [__     |___ | |    |___    | [__        *            )!", // 4
  R"!(      *___]  |  |___ |    ___]    |    | |___ |___    | ___]       *            )!", // 5
  R"!(      *                                                            *            )!", // 6
  R"!(      *_  _ _ ____ ____ _ _  _ ____                                *            )!", // 7
  R"!(      *|\/| | [__  [__  | |\ | | __                                *            )!", // 8
  R"!(      *|  | | ___] ___] | | \| |__]                                *            )!", // 9
  R"!(      *                                                            *            )!", // 10
  R"!(      *_  _ ____ _  _ _ _  _ ____    ___ ____    _  _ ____ _  _ ___*            )!", // 11
  R"!(      *|\/| |  | |  | | |\ | | __     |  |  |    |\ | |___  \/   | *            )!", // 12
  R"!(      *|  | |__|  \/  | | \| |__]     |  |__|    | \| |___ _/\_  | *            )!", // 13
  R"!(      *                                                            *            )!", // 14
  R"!(      *_    ____ _  _ ____ _                                       *            )!", // 15
  R"!(      *|    |___ |  | |___ |                                       *            )!", // 16
  R"!(      *|___ |___  \/  |___ |___                                    *            )!", // 17
  R"!(      **************************************************************            )!", // 18
  R"!(                                                                                )!", // 19
  R"!(                                                                                )!", // 20
  R"!(                                                                                )!", // 21
...
char Display::step_file_missing_save[SCREEN_HEIGHT][SCREEN_WIDTH + 1]
{
    //01234567890123456789012345678901234567890123456789012345678901234567890123456789
  R"!(                                                                                )!", // 0
  R"!(                                                                                )!", // 1
  R"!(                                                                                )!", // 2
  R"!(     ***********************************************************************    )!", // 3
  R"!(     *____ ___ ____ ___  ____    ____ _ _    ____    _ ____                *    )!", // 4
  R"!(     *[__   |  |___ |__] [__     |___ | |    |___    | [__                 *    )!", // 5
  R"!(     *___]  |  |___ |    ___]    |    | |___ |___    | ___]                *    )!", // 6
  R"!(     *                                                                     *    )!", // 7
  R"!(     *_  _ _ ____ ____ _ _  _ ____                                         *    )!", // 8
  R"!(     *|\/| | [__  [__  | |\ | | __                                         *    )!", // 9
  R"!(     *|  | | ___] ___] | | \| |__]                                         *    )!", // 10
  R"!(     *                                                                     *    )!", // 11
  R"!(     *____ ____ _  _ ___ _ _  _ _  _ ____    _ _ _ _ ___ _  _ ____ _  _ ___*    )!", // 12
  R"!(     *|    |  | |\ |  |  | |\ | |  | |___    | | | |  |  |__| |  | |  |  | *    )!", // 13
  R"!(     *|___ |__| | \|  |  | | \| |__| |___    |_|_| |  |  |  | |__| |__|  | *    )!", // 14
  R"!(     *                                                                     *    )!", // 15
  R"!(     *____ ____ _  _ _ _  _ ____                                           *    )!", // 16
  R"!(     *[__  |__| |  | | |\ | | __                                           *    )!", // 17
  R"!(     *___] |  |  \/  | | \| |__]                                           *    )!", // 18
  R"!(     ***********************************************************************    )!", // 19
  R"!(                                                                                )!", // 20
...
char Display::result_file_missing_save[SCREEN_HEIGHT][SCREEN_WIDTH + 1]
{
  //01234567890123456789012345678901234567890123456789012345678901234567890123456789
R"!(                                                                                )!", // 0
R"!(                                                                                )!", // 1
...
R"!(                                                                                )!", // 3
R"!(    ***********************************************************************     )!", // 4
R"!(    *____ ____ ____ _  _ _    ___ ____    ____ _ _    ____    _ ____      *     )!", // 5
R"!(    *|__/ |___ [__  |  | |     |  [__     |___ | |    |___    | [__       *     )!", // 6
R"!(    *|  \ |___ ___] |__| |___  |  ___]    |    | |___ |___    | ___]      *     )!", // 7
R"!(    *                                                                     *     )!", // 8
R"!(    *_  _ _ ____ ____ _ _  _ ____                                         *     )!", // 9
R"!(    *|\/| | [__  [__  | |\ | | __                                         *     )!", // 10
R"!(    *|  | | ___] ___] | | \| |__]                                         *     )!", // 11
R"!(    *                                                                     *     )!", // 12
R"!(    *____ ____ _  _ ___ _ _  _ _  _ ____    _ _ _ _ ___ _  _ ____ _  _ ___*     )!", // 13
R"!(    *|    |  | |\ |  |  | |\ | |  | |___    | | | |  |  |__| |  | |  |  | *     )!", // 14
R"!(    *|___ |__| | \|  |  | | \| |__| |___    |_|_| |  |  |  | |__| |__|  | *     )!", // 15
R"!(    *                                                                     *     )!", // 16
R"!(    *____ ____ _  _ _ _  _ ____    ____ ____ ____ _  _ _    ___ ____      *     )!", // 17
R"!(    *[__  |__| |  | | |\ | | __    |__/ |___ [__  |  | |     |  [__       *     )!", // 18
R"!(    *___] |  |  \/  | | \| |__]    |  \ |___ ___] |__| |___  |  ___]      *     )!", // 19
R"!(    ***********************************************************************     )!", // 20
R"!(                                                                                )!", // 21
R"!(                                                                                )!", // 22
R"!(                                                                                )!", // 23
...
char Display::result_file_missing_load[SCREEN_HEIGHT][SCREEN_WIDTH + 1]
{
     //01234567890123456789012345678901234567890123456789012345678901234567890123456789
   R"!(                                                                                )!", // 0
   R"!(                                                                                )!", // 1
   ...
   R"!(    *|___ |__| | \|  |  | | \| |__| |___    |_|_| |  |  |  | |__| |__|  | *     )!", // 15
   R"!(    *                                                                     *     )!", // 16
   R"!(    *____ ____ _  _ ___  ____ ____ _ _  _ ____                            *     )!", // 17
   R"!(    *|    |  | |\/| |__] |__| |__/ | |\ | | __                            *     )!", // 18
   R"!(    *|___ |__| |  | |    |  | |  \ | | \| |__]                            *     )!", // 19
   R"!(    ***********************************************************************     )!", // 20
   R"!(                                                                                )!", // 21
